...
import { auth } from "@/config/firebase/firebase"; // 클라이언트 SDK
import { adminAuth } from "./config/firebase/firebaseadmin"; // Admin SDK
import { getUserById } from "./app/(my-page)/my-page/[id]/actions";
import { createAccountHandler } from "./app/(auth)/create-account/actions";
  ...
  providers: [
    ...
    CredentialsProvider({
      ...
      authorize: async (credentials) => {
        ...
        try {
          // 서버 측 검증: Admin SDK를 사용하여 이메일로 사용자 조회
          const userRecord = await adminAuth.getUserByEmail(email);

          // 클라이언트 인증: 비밀번호 검증은 별도로 처리
          const userCredential = await signInWithEmailAndPassword(
            auth,
            email,
            password
